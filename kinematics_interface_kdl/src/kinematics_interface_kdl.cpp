  ...
  const std::string &link_name, Eigen::Matrix<double, 6, 1> & delta_x)
{
  // verify inputs
  if (
    !verify_initialized() || !verify_joint_vector(joint_pos) || !verify_link_name(link_name) || !verify_joint_vector(delta_theta))
  {
    return false;
  }
...
bool KDLKinematics::verify_joint_vector(const Eigen::Matrix<double, Eigen::Dynamic, 1> &joint_vector)
{
