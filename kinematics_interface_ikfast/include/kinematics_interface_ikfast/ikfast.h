...
public:
  ...
  unsigned char
    maxsolutions;  ///< max possible indices, 0 if controlled by free index or a free joint itself
  unsigned char indices
    [5];  ///< unique index of the solution used to keep track on what part it came from. sometimes a solution can be repeated for different indices. store at least another repeated root
};
...
public:
  ...
  virtual void Validate() const
  {
    for (size_t i = 0; i < _vbasesol.size(); ++i)
    {
      ...
      if (_vbasesol[i].maxsolutions > 0)
      {
        ...
        if (
          _vbasesol[i].indices[1] != (unsigned char)-1 &&
  ...
  std::vector<IkSingleDOFSolutionBase<T> >
    _vbasesol;  ///< solution and their offsets if joints are mimicked
